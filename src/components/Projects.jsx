            ...
            <div className='w-fit'>
                <Link to={`/ProjectDetail/${project.id}`} key={project.id}>
                    <div className="image-container">
                        <img src={project.img_url} alt={project.title} className=' w-full h-full hover:opacity-60 transition duration-300 rounded rounded-tr-[16px] object-cover  flex-none relative' />
    ...
    return (
        <div className=' mt-10 scroll-mt-40' id='project'>
            ...
            <div className='grid lg:grid-cols-3 md:grid-cols-2 grid-cols-1 gap-4 items-start'>
                {
                    (project && loading) ? (project.map((project) => (<Projectcard key={project.id} project={project} />)))
                        : (
                            <>
                                <Loadingcard />
                                <Loadingcard />
